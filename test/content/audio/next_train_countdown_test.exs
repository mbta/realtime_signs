defmodule Content.Audio.NextTrainCountdownTest do
  ...
  describe "from_predictions_message/1" do
    ...
    test "Converts Alewife countdown message on the at JFK/UMass (specifying platform) to audio" do
      message = %Content.Message.Predictions{headsign: "Alewife", minutes: 5}
      assert Content.Audio.NextTrainCountdown.from_predictions_message(message, :arrives, :ashmont) ==
        %Content.Audio.NextTrainCountdown{destination: :alewife, verb: :arrives, minutes: 5, platform: :ashmont}
    end
    ...
    test "Logs unknown headsign countdown message" do
      ...
      log = capture_log [level: :warn], fn ->
        assert Content.Audio.NextTrainCountdown.from_predictions_message(message, :arrives, nil) == nil
      end
    ...
    test "Does not announce train one minute away" do
      ...
      assert Content.Audio.NextTrainCountdown.from_predictions_message(message, :arrives, nil) == nil
      assert Content.Audio.NextTrainCountdown.from_predictions_message(message, :departs, nil) == nil
    end
