defmodule Sign.Static.Announcements do
  ...
  @spec from_schedule_headways(%{Station.t => ScheduleHeadway.t}, DateTime.t, Bridge.Chelsea.status) :: [Message.t]
  ...
  defp mid_for_bridge(_duration, :spansish), do: 152
  ...
  defp variables_for_headway(headway, current_time, language) do
    id_modifier = if language == :spanish, do: 37000, else: 5500
    do_variables_for_headway(headway, current_time, id_modifier)
  end

  defp do_variables_for_headway({:first_departure, {x, y}, first_departure}, current_time, id_modifier) do
    max_headway = ScheduleHeadway.max_headway({x, y})
    if ScheduleHeadway.show_first_departure?(first_departure, current_time, max_headway) do
      Enum.sort([x + id_modifier, y + id_modifier])
    else
      []
    end
  end
  defp do_variables_for_headway({:last_departure, time}, _current_time, id_modifier) do
  [time.minute + id_modifier]
  end
